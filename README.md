...
- **The finding this project actually stands behind: production per cycle drops as a well cycles.** qi (peak rate) declines cycle-over-cycle in about 70% of wells, per-cycle recovery in about 60%, checked both within-well and cross-sectionally, across the 609 wells with at least a 1st and 2nd high-confidence cycle, 433 of them Mahihkan, the older of the two batteries, where a "2nd cycle" is more likely to be a genuine re-steam rather than an artifact of where the 2022 data window happens to start (see the caveat below).
- **"Cycle 1" isn't always a real steam cycle, read cycle numbers carefully.** For a well whose entire production history starts at the 2022 data horizon, the startup ramp is correctly excluded, but what's left after it still gets fit and labeled "cycle 1", even when the well never actually went through a second steam job inside this window. It's really that well's initial-production decline, wearing cycle terminology built for wells with a visible re-stimulation. Concretely: this project's own top-NPV well (`ABWI105110806603W400`, $40.2M) came online 2024-05, has produced continuously on a gently declining plateau since, and shows no re-steam pattern anywhere in the raw data, its "cycle 1" boundary is a peak-detection artifact from ordinary monthly noise (relative prominence landed at ~41%, just over the 40% cutoff), not a real re-stimulation. 581 of the 1,190 wells with a usable fit (49%) have exactly one high-confidence cycle, so this isn't an edge case, it affects a meaningful share of the dataset, mostly Nabiye, the newer battery.
- **Modelled NPV/EUR totals are a scale illustration, not the headline result.** They're real barrels, valued under this project's own illustrative cost assumptions, summed across every fitted cycle including "cycle 1"s that are initial decline rather than re-stimulation (see above), and skewed by a handful of standout wells, the top 10 wells alone account for 14.5% of total modelled NPV. Useful for showing the pipeline can price what it detects end to end, not for reading "modelled NPV: $1.58B" as an asset valuation.
- Negative-NPV share rises with cycle position (17.5% at cycle 1 up to 25.2% at cycle 3+), consistent with the degradation trend. That said, this is largely mechanical: steam cost is a flat $200k per cycle regardless of well size, so at current prices "negative NPV" is close to "this cycle produced under ~3,700 bbl", later cycles produce less, so more of them fall under that line.
...
- IRR can return extreme, not-economically-meaningful values (thousands of percent) for cycles with a very small or front-loaded cost basis relative to revenue, since it's a root-finding solution over sparse monthly cash flows, not a bounded metric. NPV is the more reliable number in this dashboard; IRR is shown alongside it, not in place of it. IRR also comes back undefined (not a bug) for a cycle whose cash flow never goes negative, month zero's revenue alone already covers the steam cost, which happens on several of the largest wells; the dashboard shows this as "n/a (never negative)" rather than a bare "n/a".
- **No Crown royalty, no abandonment/reclamation liability.** Modelled NPV is pre-royalty gross-project cash flow. Alberta oil sands royalty is roughly 1-9% of gross revenue pre-payout (escalating with WTI) and 25-40% of net revenue post-payout, this project doesn't model it at all, which overstates every NPV number here materially. On top of the diluent and cost-assumption caveats above, this is the single biggest reason "modelled NPV" isn't a valuation.
- All monetary figures (WTI, WCS, opex, steam cost, NPV) are USD, config.yaml and the dashboard don't say so explicitly anywhere. Alberta operating costs are normally quoted in CAD; no currency conversion is applied here.
- The R2 >= 0.5 threshold for "high confidence" is a starting guess, not a validated cutoff (documented in `decline.py`), and it's lenient enough that a real share of headline value sits in the 0.5-0.7 band, including the top well above (R2 = 0.59). Tightening to R2 >= 0.7 as a cross-check keeps 69.6% of cycles but only 67.3% of modelled NPV ($1.06B vs $1.58B), so the totals are meaningfully sensitive to where this line is drawn.
